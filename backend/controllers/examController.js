    ...
    try {
        if (role === "student") {
            ...
            user = await User.find({ _id: userId })
                ...
                .exec();// Explicitly triggers the DB call and returns a real Promise.
            //get exam submite
            submittedData = await Submission.find({ userId })
                ...
                .populate({  //.populate() is a method used to automatically replace a reference field (usually an ObjectId) with the actual document data from another collection. 
                    path: 'userId',
...
// Get exam by ID complete details of one exam along with its questions
